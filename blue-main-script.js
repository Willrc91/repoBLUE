...
// Pase ---------- PN-0005506
// DATE: --------- 04-10-2022
// DESCRIPTION: -- Define y contruye los elementos para el indicador de procesos
// AUTHOR: ------- Daniel Giménez
// WORKTEAM: ----- Merge
// version 1.0
class Steps {
    constructor(idElement, stepsNumber, stepsAlign = 'center') {
        this.idElement = '';
    ...
    init() {
        ...
        this.settings.minSteps = 2;
        this.settings.maxSteps = 7;
        this.settings.callbackThis = this;
        this.build();
    }
    build() {
        var _a;
        this.instanceElement = document.querySelector(`#${this.idElement}`);
        this.instanceElement.classList.add("steps-container");
        if (this.settings.steps < 2) {
            this.settings.steps = this.settings.minSteps;
        }
        for (var i = 0; i < this.settings.steps; i++) {
            if (i < this.settings.maxSteps) {
                this.instanceElement.append(BLUEUtils.stringToHTML('<div id="step-' + i + '" class="dots"></div>'));
            }
        }
        (_a = this.instanceElement.firstElementChild) === null || _a === void 0 ? void 0 : _a.classList.add("init-dot");
        this.instanceElement.classList.add("j-content-" + this.settings.justify);
    }
    prevPosition() {
        if (this.settings.position > 0) {
            this.settings.position--;
            document.querySelector('#step-' + (this.settings.position + 1)).classList.remove('done');
            this.stepsPosition(this.settings.position);
        }
    }
    nextPosition() {
        if (this.settings.position < (this.settings.steps - 1) &&
            this.settings.steps <= this.settings.maxSteps) {
            this.settings.position++;
            document.querySelector('#step-' + (this.settings.position - 1)).classList.add('done');
            this.stepsPosition(this.settings.position);
        }
    }
    stepsPosition(position) {
        let percentageCalc = position * 100, pxCalc = position * 14;
        document.querySelector('#step-0').style.setProperty("--transform-position", "translateX(calc(" + percentageCalc + "% + " + pxCalc + "px))");
    }
    clearPosition() {
        this.settings.position = 0;
        document.querySelector('#step-0').style.setProperty("--transform-position", "translateX(0)");
        var dotsElement = this.instanceElement.children;
        for (const element of Array.from(dotsElement)) {
            const dot = element;
